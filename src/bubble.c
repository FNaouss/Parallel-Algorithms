...
void parallel_bubble_sort(uint64_t *T, const uint64_t size) {
   int sorted;
    ...
    do {
        ...
        #pragma omp parallel for schedule(static) private(i) shared(T, chunk_size) reduction(&&:sorted)
        for (int tid = 0; tid < num_threads; tid++) {
            uint64_t start = tid * chunk_size;
            uint64_t end = (tid == num_threads - 1) ? size : start + chunk_size;
            int local_sorted = 1;
            
            //bubble sort in chunk
        ...
        for (i = chunk_size - 1; i < size - 1; i += chunk_size) {
            if (T[i] > T[i + 1]) {
                uint64_t temp = T[i];
                T[i] = T[i + 1];
                T[i + 1] = temp;
